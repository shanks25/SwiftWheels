...
/** Changes by Ruchita Starts
     ...
     * @return \Illuminate\Http\Response
     */
    public function payout_details($type = 'individual')
    {
       ...
       {
            $with_provider = $with_admin = $prov_comm = $admin_comm =  0;

            $prov_user_req = UserRequests::with('payment')->where('provider_id',$provider->id)
            ->where('status','COMPLETED')
            ->get();
            
            foreach($prov_user_req as $single_request)
            {
                if($single_request->payment->payment_mode == 'CASH')
                {
                    $with_provider = $with_provider + ($single_request->payment->total - $single_request->payment->tax);
                    $prov_comm = $prov_comm + $single_request->payment->provider_commission;
                    $admin_comm = $admin_comm + $single_request->payment->provider_pay;
                }
                else
                {
                    $with_admin = $with_admin + ($single_request->payment->total - $single_request->payment->tax);
                    $admin_comm = $admin_comm + $single_request->payment->provider_pay;
                    $prov_comm = $prov_comm + $single_request->payment->provider_commission;
                }
           }
           
                $provider->with_provider = $with_provider;
                $provider->prov_comm = $prov_comm;
                $provider->with_admin = $with_admin;
                $provider->admin_comm = $admin_comm;
          
       }

            return view('admin.payout', compact('rides', 'providers'))
                ->with('page', $page);
    ...
    public function payout_details_for_one($id)
    {
        ...
        try {
            ...
            $prov_user_req = UserRequests::with('payment')->where('provider_id',$provider_id)
            ->where('status','COMPLETED')
            ->get();
            ...
            foreach($prov_user_req as $single_request)
            {
                ...
                $single_request->with_provider = $with_provider;
                $single_request->prov_comm = $prov_comm;
                $single_request->with_admin = $with_admin;
                $single_request->admin_comm = $admin_comm;
           }
           
               
          

            return view('admin.payout_details', compact( 'prov_user_req'))
                ->with('page', $page);

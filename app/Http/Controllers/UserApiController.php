...
class UserApiController extends Controller
{
    ...
    public function signup(Request $request)
    {
        ...
        try {
            ...
            if ($request->has('has_eme_contact')) {

                $emeContact1 = new EmeContact();
                $emeContact1->user_id = $User->id;
                $emeContact1->name = $request->one_name;
                $emeContact1->phone_number = $request->one_phone_number;
                $emeContact1->save();

                $emeContact2 = new EmeContact();
                $emeContact2->user_id = $User->id;
                $emeContact2->name = $request->two_name;
                $emeContact2->phone_number = $request->two_phone_number;
                $emeContact2->save();
    ...
    public function send_request(Request $request)
    {
        ...
        $this->validate($request, [
            ...
            'vehicle_type' => '',
            'use_wallet' => 'numeric',
            'payment_mode' => 'required|in:CASH,CARD,PAYPAL,ONLN',
            'card_id' => ['required_if:payment_mode,CARD', 'exists:cards,card_id,user_id,' . Auth::user()->id],
        ]);

if($request->ride_type == '0')
        $ride_type = 4;
else
        $ride_type = 6;
        ...
        // OUTSTATION
        // todo check fot outstation ride type
        $ride_service_price = RideServicePrice::where('service_type_id', $service_type)
            ->where('ride_type_id', $ride_type)
            ->first();

        if ($request->distance >= $ride_service_price->max_distance) {

            $ride_type = 5;

            $desLatitude = $request->d_latitude;
            $desLongitude = $request->d_longitude;

            $sourceOutstation = Outstation::select(DB::Raw("(6371 * acos( cos( radians('$latitude') ) * cos( radians(latitude) ) * cos( radians(longitude) - radians('$longitude') ) + sin( radians('$latitude') ) * sin( radians(latitude) ) ) ) AS distance"), 'name', 'id')
                ->whereRaw("(6371 * acos( cos( radians('$latitude') ) * cos( radians(latitude) ) * cos( radians(longitude) - radians('$longitude') ) + sin( radians('$latitude') ) * sin( radians(latitude) ) ) ) <= $ride_service_price->max_distance")
                ->orderBy('distance', 'asc')
                ->first();

            $destinationOutstation = Outstation::select(DB::Raw("(6371 * acos( cos( radians('$desLatitude') ) * cos( radians(latitude) ) * cos( radians(longitude) - radians('$desLongitude') ) + sin( radians('$desLatitude') ) * sin( radians(latitude) ) ) ) AS distance"), 'name', 'id')
                ->whereRaw("(6371 * acos( cos( radians('$desLatitude') ) * cos( radians(latitude) ) * cos( radians(longitude) - radians('$desLongitude') ) + sin( radians('$desLatitude') ) * sin( radians(latitude) ) ) ) <= $ride_service_price->max_distance")
                ->orderBy('distance', 'asc')
                ->first();

            if ($sourceOutstation && $destinationOutstation) {
                $outstation_price = OutstationPrice::where('source_id', $sourceOutstation->id)->where('destination_id', $destinationOutstation->id)->first();

                if (!$outstation_price) {
                    $outstation_price = OutstationPrice::where('source_id', $destinationOutstation->id)->where('destination_id', $sourceOutstation->id)->first();
                    if (!$outstation_price) {
                        if ($request->ajax()) {
                            // Push Notification to User
                            return response()->json(['message' => trans('api.ride.no_service_in_location')]);

                        } else {
//                            return back()->with('flash_success', trans('api.ride.no_service_in_location'));
                        }
                    }
                }
            }
...
if( $ride_type == 6)
    ...
    ->with('trips')
            ...
            ->whereHas('rides', function ($query) use ($ride_type) {
                $query->where('ride_type_id', $ride_type);
            })
            ->orderBy('distance', 'asc')
            ->get();
    }  
...
if(($ride_type == 6) && count($Providers) == 0)
{
     $Providers = Provider::with('service')
      ->with('trips')
            ->select(DB::Raw("(6371 * acos( cos( radians('$latitude') ) * cos( radians(latitude) ) * cos( radians(longitude) - radians('$longitude') ) + sin( radians('$latitude') ) * sin( radians(latitude) ) ) ) AS distance"), 'id')
...
if(($ride_type == 6) && count($Providers) != 0)
{
       Log::info('Providers before : ' . $Providers);

for($i = 0; $i < $Providers->count();$i++){
    
    if($Providers[$i]->trips['0']['status'] == 'STARTED'){
           unset($Providers[$i]);
        }
}
    Log::info('Providers after : ' . $Providers);
}
*/
        ...
        try {
            ...
            $details = "https://maps.googleapis.com/maps/api/directions/json?origin=" . $request->s_latitude . "," . $request->s_longitude . "&destination=" . $request->d_latitude . "," . $request->d_longitude . "&mode=driving&key=AIzaSyAuxmmUPDIXgiw84E9AX7bbbdFzkd0xd50";
            ...
            $UserRequestsold = UserRequests::where('current_provider_id', $Providers[0]->id)
...
if(!$UserRequestsold){
     $UserRequest = new UserRequests;
            ...
            $UserRequest->travel_time = $request->travel_time;

            // ride type
            $UserRequest->ride_type_id = $ride_type;
            $UserRequest->shareseat = $request->seats;
    ...
    /*        if ($Providers->count() <= Setting::get('surge_trigger') && $Providers->count() > 0) {
            ...
            if (Setting::get('manual_request', 0) == 0) {
                foreach ($Providers as $key => $Provider) {
                   ...
                   $ser_type = ServiceType::where('id', $request->service_type)->first();

if($ser_type->share_type == '1')
{
    $veh_type = VehicleType::where(strtolower('type'), strtolower($request->vehicle_type))->first();
    $avail_seats  = $veh_type->seating_capacity - $request->seats;
}
else
{
    $avail_seats= 0;
                    ...
                    $Filter->booked_seats  = $request->seats;
                    $Filter->available_seats  = $avail_seats;
...
else
{
    //
     return back()->with('flash_success', 'No Providers Found! Please try again.');
            ...
            Log::info('exception from send request catch file: ' . $e);
    ...
    public function estimated_fare(Request $request)
    {
        ...
        try {
            ...
            $details = "https://maps.googleapis.com/maps/api/distancematrix/json?origins=" . $request->s_latitude . "," . $request->s_longitude . "&destinations=" . $request->d_latitude . "," . $request->d_longitude . "&mode=driving&sensor=false&key=AIzaSyAuxmmUPDIXgiw84E9AX7bbbdFzkd0xd50";
            ...
            $ride_service_price = RideServicePrice::where('service_type_id', $service_type->id)
                ->where('ride_type_id', $ride_type)
                ->first();

            //
            $basePrice = $ride_service_price->fixed;
            $minuteCharges = $ride_service_price->minute;
            $distanceCharges = $ride_service_price->price;

            //
            $totalMinute = $minutes;
            $totalHour = $minutes * 60;
            $totalDistance = $kilometer - $ride_service_price->distance;

            //
            $totalMinuteCharges = $totalMinute * $minuteCharges;
            $totalHourCharges = $totalHour * $minuteCharges;
            $totalDistanceCharges = $totalDistance * $distanceCharges;

            if ($kilometer >= $ride_service_price->max_distance) {

                $ride_type = 5;

                $sourceLatitude = $request->s_latitude;
                $sourceLongitude = $request->s_longitude;
                $desLatitude = $request->d_latitude;
                $desLongitude = $request->d_longitude;

                $sourceOutstation = Outstation::select(DB::Raw("(6371 * acos( cos( radians('$sourceLatitude') ) * cos( radians(latitude) ) * cos( radians(longitude) - radians('$sourceLongitude') ) + sin( radians('$sourceLatitude') ) * sin( radians(latitude) ) ) ) AS distance"), 'name', 'id')
                    ->whereRaw("(6371 * acos( cos( radians('$sourceLatitude') ) * cos( radians(latitude) ) * cos( radians(longitude) - radians('$sourceLongitude') ) + sin( radians('$sourceLatitude') ) * sin( radians(latitude) ) ) ) <= $ride_service_price->max_distance")
                    ->orderBy('distance', 'asc')
                    ->first();
...
//                            return back()->with('flash_success', trans('api.ride.no_service_in_location'));
                ...
                //
                $basePrice = $outstation_price->fixed;
                $distanceCharges = $outstation_price->price;

                //
                $totalDistance = $kilometer - $outstation_price->distance;
            ...
            switch ($ride_service_price->calculator) {
                case 'MIN':
                    $price = $basePrice + $totalMinuteCharges;
                    break;
                case 'HOUR':
                    $price = $basePrice + $totalHourCharges;
                    break;
                case 'DISTANCE':
                    $price = $basePrice + $totalDistanceCharges;
                    break;
                case 'DISTANCEMIN':
                    $price = $basePrice + $totalDistanceCharges + $totalMinuteCharges;
                    break;
                case 'DISTANCEHOUR':
                    $price = $basePrice + $totalDistanceCharges + $totalHourCharges;
                    break;
                default:
                    $price = $basePrice + $totalDistanceCharges;
                    break;
...
//                'minute_charge' => $ride_service_price->minute,
//                'base_distance' => $outstation_price->distance,
//                'distance_charge' => $distanceCharges,
...
//            ]);
    ...
    {
        try {
                if ($serviceList = VehicleType::all()) {
                    return $serviceList;
                } else {
                    return response()->json(['error' => trans('api.services_not_found')], 500);
                }
 ...
 public function get_vehicle_seats(Request $request)
    {
    
        
        try {
               $seat_array= array();
                $seats =  VehicleType::where('id',$request->id)->first();
                $no=  $seats['seating_capacity'];
              for($i=0; $i< $no; $i++){
                  $seat_array[$i]= array();
                    $seat_array[$i]= $i+1;
                } 
                 if ($seat_array) {
                         return response()->json(['vehicle_seats' => $seat_array ]);

...
@section('content')
    <div class="content-area py-1">
        <div class="container-fluid">
                ...
                <div class="row">
                    ...
                    <form action="{{ route('admin.provider.document.store', $Provider->id) }}" method="POST">
                        ...
                        <div class="col-xs-3">
                         ...
                         <div class="col-xs-3">
                            <select class="form-control input" name="vehicle_type" required>
            ...
            <div class="box box-block bg-white">
                <h5 class="mb-1">Provider Ride Type Allocation</h5>
                <div class="row">
                    <div class="col-xs-12">
                        @if($providerRides->count() > 0)
                            <hr><h6>Allocated Ride Types : </h6>
                            <table class="table table-striped table-bordered dataTable">
                                <thead>
                                <tr>
                                    <th>Ride Type Name</th>
                                    <th>@lang('admin.action')</th>
                                </tr>
                                </thead>
                                <tbody>
                                @foreach($providerRides as $ride)
                                    <tr>
                                        <td>{{ $ride->ride_type->name }}</td>
                                        <td>
                                            <form action="{{ route('admin.provider.delete_assign_ride', [$Provider->id, $ride->ride_type->id]) }}"
                                                  method="POST">
                                                {{ csrf_field() }}
                                                {{ method_field('DELETE') }}
                                                <button class="btn btn-danger btn-large btn-block">Delete</button>
                    ...
                    <form action="{{ route('admin.provider.assign_ride', $Provider->id) }}" method="POST">
                        {{ csrf_field() }}
                        <div class="col-xs-3">
                            <select class="form-control input" name="ride_type" required>
                                @forelse($rideTypes as $rideType)
                                    <option value="{{ $rideType->id }}">{{ $rideType->name }}</option>
                                @empty
                                    <option>- Please Create a Ride Type -</option>
                                @endforelse
                            </select>
                        </div>
                        <div class="col-xs-3">
                            <button class="btn btn-primary btn-block" type="submit">Add Ride Type</button>
                        </div>
                    </form>
                </div>
            </div>
